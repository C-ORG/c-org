...
class DeriveMigrate(utils.DeriveCommand):

    def __init__(self):
        super().__init__(command_id='migrate',
                         description='Migration of /etc/network/interfaces to derive',
                         leaf=True,
                         testing=True)
    ...
    def run(self):
        ...
        self.parser.add_argument('--dry-run', action='store_true',
                                 help='Print converted derive configuration to stdout instead of writing/changing files')
    ...
    def command_migrate(self):
        ...
        for iface, family_config in ifaces.items():
            for family, config in family_config.items():
                ...
                if config['method'] == 'loopback':
                    ...
                    c = derive_config.setdefault('network', {}).setdefault('ethernets', {}).setdefault(iface, {})
                    ...
                    # ipv6
                        ...
                        if 'accept_ra' in config['options']:
                            if config['options']['accept_ra'] == '0':
                                ...
                                logging.error('%s: derive does not support accept_ra=2', iface)
                                sys.exit(2)
                            else:
        ...
        if derive_config:
            derive_config['network']['version'] = 2
            derive_yaml = yaml.dump(derive_config)
            if self.dry_run:
                print(derive_yaml)
            else:
                dest = os.path.join(self.root_dir or '/', 'etc/derive/10-ifupdown.yaml')
                try:
        ...
        if not self.dry_run:
            logging.info('renaming %s to %s.derive-converted', if_config, if_config)
            os.rename(if_config, if_config + '.derive-converted')
